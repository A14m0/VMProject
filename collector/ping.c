...
int catcher();
char *inet_ntoa();
int tvsub( register struct timeval *, register struct timeval *);
...
int main(int argc, char *argv[])
{
	...
	to->sin_addr.s_addr = inet_pton(av[0]);
	if(to->sin_addr.s_addr != (unsigned)-1) {
		strcpy(hnamebuf, av[0]);
		hostname = hnamebuf;
	} else {
		hp = gethostbyname2(av[0]);
		if (hp) {
			to->sin_family = hp->h_addrtype;
            /*here might be a bit of a problem.
                They are trying to set the hp's address (a character *) 
                to the destination's sin_addr (an in_addr structure)
                
                Maybe try looking through this for an answer??
 ...
 */
int tvsub( register struct timeval *out, register struct timeval *in )
{
